...
#
# Anonymous by construction: an identity-free Starship-clone shell prompt and a
# cockpit theme that OWNS both status rows (the default tmux status-right prints
# the machine's hostname — the cockpit format replaces it so nothing leaks).
#
...
# pills (demo chrome), Row 2 right = the FLOAT capsule.
#
# HONEST BY DESIGN: that FLOAT capsule is driven by this plugin's REAL live
# state via docs/fp-status.sh (#() calls) — the scratch session name, the float
# geometry, and its show/ready state, read straight from the fp-demo server.
# (Tokens live in status-format here, which floatpane.tmux does not rewrite, so
# the reader is wired directly.)
set -u
...
# ── Row 2: quota trio (left, staged chrome) · FLOAT capsule (right, driven by
#    this plugin's REAL live state via docs/fp-status.sh, wired directly since
#    status-format isn't rewritten) ──
...
ROW2_R="$(p_open "$TEAL" "$I_TERM")$(p_text "#('$FPSTAT' session)")$(p_ibadge "$GREEN" "$I_FLEET")$(p_text "#('$FPSTAT' size)")$(p_ibadge "$YELLOW" "$I_PLAY")$(p_text "#('$FPSTAT' state)")$(p_close)"
...
#    clean Starship rc) — the toggle then just attaches to it ──
